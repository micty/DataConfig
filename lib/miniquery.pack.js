...
* @param {boolean} [isStrict] 指定是否要进行严格判断，如果是请传入 true；否则当成非严格判断
...
MiniQuery.Script = 
{
    /**
    * 跨浏览器动态加载 JS。
    * 可以批量加载多个 JS，并在加载完最后一个之后执行回调。
    */
    load: function(url, charset/*可选*/, fn)
    {
        //多文件串联加载
        if( MiniQuery.Array.hasItem(url) ) //表示是一个数组
        {
            var load = arguments.callee; //指向当前函数，即使外部改名了，也不影响内部引用
            
            var urls = url; //改名成数组的命名更好理解
            var index = 0;
            ...
            (function()
            {
                ...
                load( urls[index], charset, function()
                {
                    index++;
        ...
        if(typeof charset == 'function') //此时为 MiniQuery.Script.load(url, fn);
        {
            fn = charset;
            charset = null;
        }
        ...
        if(script.readyState) //IE
        {
            script.onreadystatechange = function()
            {
                if(script.readyState == 'loaded' || script.readyState == 'complete')
                {
                    ...
                    head.removeChild(script); //已加载完毕，移除该 script 标签
                    fn && fn();
                }
            };
        }
    ...
    write: function(src, charset)
    {
        var $ = MiniQuery;
        
        document.write($.String.format('<script type="text/javascript" src="{src}" {charsetProperty} ><\/script>', 
        {
            src: src,
            charsetProperty: charset ? $.String.format('charset="{0}"', charset) : ''
        }));
    }
};
